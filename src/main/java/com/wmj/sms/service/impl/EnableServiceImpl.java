...
public class EnableServiceImpl extends ServiceImpl<EnableMapper, Enable> implements EnableService {
            ...
            if (selectStudent != null) {
                if (loginUser.getId().equals(selectStudent.getManId())) {
                    ...
                    if (super.getOne(wrapper) != null) {
                        return JsonResponse.failure(selectStudent.getStuName() + "学生已经认定为贫困生，请勿重复添加");
                    }
    ...
    @Override
    public JsonResponse delete(Student student) {
        User loginUser = SessionUtils.getCurUser();
        if (loginUser.getUserType().equals("3")) {
            Student selectStudent = studentService.getByStuId(student.getStuId());
            if (selectStudent != null) {
                if (loginUser.getId().equals(selectStudent.getManId())) {
                    ...
                    if (super.getOne(wrapper) != null) {
                        ...
                        stuApply.setStuId(selectStudent.getStuId());
                        stuApplyService.deleteApplyByStuId(stuApply);
                        return JsonResponse.successMessage( "移出贫困生成功");
                    }
                    return JsonResponse.failure("移出失败，该学生非贫困生");
                } else {
                    return JsonResponse.failure("移出失败，您非本学生的辅导员");
                }
            } else {
                return JsonResponse.failure("移出失败，不存在该学生");
            }
        }
        return JsonResponse.failure("移出失败，您无本操作权限");
    }

    @Override
    public JsonResponse getEnableList() {
        return JsonResponse.success(super.list(),"查询成功");
    }
}
